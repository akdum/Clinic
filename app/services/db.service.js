                ...
                DbService.prototype.getDoctors = function () {
                    ...
                    return new Promise(function (resolve, reject) { return _this._dynamoDB.scan(params, function (err, data) {
                        if (err == null) {
                            ...
                            if (data.Count > 0) {
                                for (var index = 0; index < data.Count; index++) {
                                    returnItems.push(new doctor_1.Doctor(_this._utilities.getStringFromField(data.Items[index].Name), _this._utilities.getListStringsFromField(data.Items[index].Speciality), _this._utilities.getStringFromField(data.Items[index].Url), _this._utilities.getStringFromField(data.Items[index].PhotoName), _this._utilities.getListTextFromField(data.Items[index].Text)));
                                }
                            }
                            resolve(returnItems);
                        }
